// server/scripts/seedIncremental.js
...
async function seedIncremental() {
    try {
        ...
        const existingMemeIds = new Set(
            (await Meme.find({}, 'id').lean()).map(meme => meme.id)
        );

        // Filter out only new memes
        const newMemes = dummyMemes.filter(meme => !existingMemeIds.has(meme.id));

        if (newMemes.length === 0) {
            console.log('No new memes to insert');
            return;
        }

        console.log(`Found ${newMemes.length} new memes to insert`);

        // Insert only new memes
        const insertedMemes = await Meme.insertMany(newMemes.map(meme => ({
            ...meme,
        ...
        const existingProjectNames = new Set(
            await Project.distinct('name')
        );

        // Get unique new projects from new memes
        const newProjectNames = [...new Set(
            newMemes.map(meme => meme.projectName)
        )].filter(name => !existingProjectNames.has(name));

        console.log(`Found ${newProjectNames.length} new projects to insert`);

        // Create only new projects
        const projectPromises = newProjectNames.map(projectName => {
            ...
            return Project.create({
                name: projectName,
                description: firstMeme.projectDetails.description,
                projectType: firstMeme.projectDetails.projectType,
        ...
        // Log results
        console.log(`Successfully inserted ${insertedMemes.length} new memes`);
        console.log(`Successfully inserted ${newProjectNames.length} new projects`);

        // Optional: Log the IDs of new memes for verification
        console.log('Newly inserted meme IDs:', newMemes.map(m => m.id));
        console.log('Newly inserted projects:', newProjectNames);
